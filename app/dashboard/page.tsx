...
const CATS = [
  {
    ...
    imgUrl: 'https://images.unsplash.com/photo-1631679706909-1844bbd07221?w=800&q=80',
    tag: 'Parede e Teto',
  },
  {
    id: 'cristal',
    ...
    imgUrl: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=800&q=80',
  ...
  },
  {
    ...
    imgUrl: 'https://images.unsplash.com/photo-1570129477492-45c003edd2be?w=800&q=80',
    tag: 'Uso Externo',
  },
  {
    id: 'granulado',
    ...
    <div className="min-h-screen bg-[#FAF7F4]">
      ...
      <div className="bg-[#2C2520] px-6 py-10 sm:px-10">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-serif text-white">Olá, {user?.nome?.split(' ')[0]}.</h1>
            <p className="text-sm text-white/50 mt-1">{user?.boutiqueName}</p>
          </div>
          <Link href="/especificacao"
            ...
            {CATS.map(cat => (
              <Link key={cat.id} href={`/especificacao?cat=${cat.id}`}
                ...
                <div className="relative h-44 overflow-hidden">
                  <img src={cat.imgUrl} alt={cat.nome}
          ...
          {loading ? (
            ...
            <div className="bg-white border border-[#E8DFD0] rounded-xl overflow-hidden">
              {sims.map((s: any, i: number) => {
                ...
                return (
                  <div key={s.id} className={`flex items-center justify-between px-5 py-3.5 ${i < sims.length-1 ? 'border-b border-[#F0E8DC]' : ''}`}>
                    ...
                    <div className="text-right ml-4 shrink-0">
                      {res?.areaComPerda && <p className="text-sm font-medium text-[#2C2520]">{res.areaComPerda.toFixed(1)} m²</p>}
