...
import { ResultadoCalculo } from '@/lib/calculo'
...
export default async function HistoricoPage() {
  ...
  const totalGasto = sims.reduce((acc, s) => {
    const r = s.resultado as ResultadoCalculo
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {sims.length === 0 ? (
          ...
          <div className="space-y-2">
            {sims.map(s => {
              const res = s.resultado as ResultadoCalculo
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs bg-[#F5EFE0] text-[#8B6B4A] px-2 py-0.5 rounded-full font-medium">
                          {CAT_LABEL[s.categoria] || s.categoria}
                        </span>
                        {session.perfil === 'admin' && (
                          <span className="text-xs text-[#6B5A4E]">{s.nomeBoutique}</span>
                        )}
                      </div>
                      <p className="text-sm font-medium text-[#2C2520] truncate">{principal?.nome || s.produto}</p>
                    ...
                    <div className="text-right shrink-0">
                      <div className="flex gap-4 mb-1">
                        {res?.areaComPerda && (
                          <div>
                            <p className="text-xs text-[#6B5A4E]">Área</p>
                            <p className="text-sm font-medium">{res.areaComPerda.toFixed(1)} m²</p>
                          </div>
                        )}
                        {principal?.quantidade && (
                          <div>
                            <p className="text-xs text-[#6B5A4E]">Qtd</p>
                            <p className="text-sm font-medium">{principal.quantidade} {principal.unidade?.split('(')[0].trim()}</p>
                          </div>
                        )}
                        {res?.totalGeral && res.totalGeral > 0 ? (
                          <div>
                            <p className="text-xs text-[#6B5A4E]">Total</p>
                            <p className="text-sm font-medium text-[#C4A882]">{fmt(res.totalGeral)}</p>
                          </div>
                        ) : null}
                      </div>
