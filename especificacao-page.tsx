...
// ─── Paleta de cores do Cimento Queimado (da cartela oficial Tez) ─────────────
...
const CORES_LUNAR_PALETTE: Record<string, { hex: string }> = {
  'Polar':     { hex: '#F2F2F0' },
  'Luar':      { hex: '#C8CFCB' },
  'Concreto':  { hex: '#8E9491' },
  'Asfalto':   { hex: '#696E6B' },
...
const CATS = [
  {
    ...
    desc: 'Matte e Lunar',
    imgUrl: 'https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?w=600&q=80',
  },
  {
    id: 'cristal',
    nome: 'Cristal de Pedras Naturais',
    desc: '#10 e #20',
    imgUrl: 'https://images.unsplash.com/photo-1615971677499-5467cbab01b0?w=600&q=80',
  },
  {
    id: 'drenante',
  ...
  },
  {
    ...
    nome: 'Granulados',
    desc: '9 cores',
    imgUrl: 'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=600&q=80',
  },
]
...
function EspecificacaoInner() {
  ...
  // Quando muda tipoBase do drenante, ajusta automaticamente primer e tela
  useEffect(() => {
    if (cat !== 'drenante') return
    // Tela sempre ligada por padrão
    setUsarTela(true)
    setMostrarAvisoTela(false)
    // Primer obrigatório para contrapiso
  ...
  function handleDesmarcarPrimer() {
    const primerObrigatorio = tipoBase === 'Contrapiso Leve' || tipoBase === 'Contrapiso Pesado'
    setUsarPrimer(false)
    if (primerObrigatorio) setMostrarAvisoPrimer(true)
  }
  ...
  async function calcular() {
    ...
    const entrada = {
      ...
      tipoAplicacao: 'Parede/Teto', // fixo para cimento
      ambiente: 'Interno',          // fixo para cimento
      tipoBase, tipoTrafego,
      area: areaNum, perda,
      usarPrimer, usarFundo, usarTela, usarCera,
      consumoPrimer,
      precos,
    }
  ...
  function nova() {
    ...
    setPerda(10); setPrecos({}); setResultado(null); setSalvo(false)
  ...
  // ── UI helpers ──────────────────────────────────────────────────────────────
  const radioBtn = (label: string, active: boolean, onClick: () => void, danger = false) => (
    <button key={label} onClick={onClick}
      className={`px-3 py-1.5 rounded-full text-sm border transition-colors ${active
        ? danger ? 'bg-red-700 border-red-700 text-white' : 'bg-[#2C2520] border-[#2C2520] text-white'
        : 'border-[#E8DFD0] text-[#6B5A4E] hover:border-[#C4A882]'}`}>
      {label}
    </button>
  )
  ...
  const fieldLabel = (text: string, sub?: string) => (
    <label className="block text-xs font-medium text-[#6B5A4E] tracking-wider uppercase mb-1.5">
      {text}
      {sub && <span className="ml-2 normal-case font-normal text-[#6B5A4E]/60">{sub}</span>}
    </label>
  )
  ...
  const avisoCard = (texto: string) => (
    <div className="p-3 bg-amber-50 border border-amber-300 rounded-xl text-xs text-amber-900 leading-relaxed flex gap-2">
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {(linha === 'Matte' ? CORES_MATTE : CORES_LUNAR).map(nome => {
                  const palette = linha === 'Matte' ? CORES_MATTE_PALETTE : CORES_LUNAR_PALETTE
                  return (
                    <ColorSwatch
                      key={nome}
                      nome={nome}
                      hex={palette[nome]?.hex || '#999'}
                      selected={cor === nome}
                      onClick={() => setCor(nome)}
                    />
                  )
            ...
            <div>
              ...
              <label className="flex items-center gap-3 p-3 bg-white border border-[#E8DFD0] rounded-xl cursor-pointer hover:border-[#C4A882] transition-colors">
                <input
                  type="checkbox"
                  checked={usarCera}
                  onChange={e => setUsarCera(e.target.checked)}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['Solo Natural','Contrapiso Leve','Contrapiso Pesado','Concreto Drenante'].map(b =>
                radioBtn(b, tipoBase===b, () => setTipoBase(b))
              )}
            </div>
          </div>
          <div>
          ...
          {/* Complementares com regras automáticas */}
          <div>
            {fieldLabel('Complementares')}
            <div className="space-y-2">
              {/* Tela — sempre marcada por padrão */}
              <div className="p-3 bg-white border border-[#E8DFD0] rounded-xl">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    checked={usarTela}
                    onChange={e => {
                      if (!e.target.checked) handleDesmarcarTela()
                      else { setUsarTela(true); setMostrarAvisoTela(false) }
                    }}
                    className="w-4 h-4 accent-[#D4875A]"
                  />
              ...
              {/* Primer PU — obrigatório para contrapiso */}
              <div className="p-3 bg-white border border-[#E8DFD0] rounded-xl">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={usarPrimer}
                    onChange={e => {
                      if (!e.target.checked) handleDesmarcarPrimer()
                      else { setUsarPrimer(true); setMostrarAvisoPrimer(false) }
                    }}
                    className="w-4 h-4 accent-[#D4875A]"
                  />
                  <div className="flex-1">
                ...
                {/* Consumo de primer — só mostra se marcado */}
                {usarPrimer && (
  ...
  if (step === 3) return (
    <div>
      ...
      <div className="space-y-4">
        ...
        {/* Fator de perda — presets simplificados sem texto abaixo */}
        <div>
          {fieldLabel(`Fator de perda — ${perda}%`)}
          <div className="flex gap-2 mt-1">
            {perdaPresets.map(([v, label]) => (
              <button key={v} onClick={() => setPerda(Number(v))}
                className={`flex-1 py-2 rounded-xl text-sm border transition-colors ${perda === Number(v)
        ...
        <div className="border-t border-[#E8DFD0] pt-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {cat === 'cimento' && <>
              {precoInput('Cimento Queimado (balde)', 'principal')}
              {usarCera && precoInput('Cera Acrílica', 'cera')}
            </>}
            {cat === 'cristal' && <>
              {precoInput('Cristal (balde 23 kg)','principal')}
              {usarPrimer && precoInput('Primer Cristal','primer')}
              {usarFundo && precoInput('Fundo Preparador','fundo')}
            </>}
            {cat === 'drenante' && <>
              {precoInput('Piso Drenante (kit 16 kg)','principal')}
              {usarPrimer && precoInput('Primer PU (4 kg)','primer')}
              {usarTela && precoInput('Tela de Fibra (rolo)','tela')}
            </>}
            {cat === 'granulado' && <>
              {precoInput('Granulado (balde 23 kg)','principal')}
              {usarPrimer && precoInput('Primer','primer')}
            </>}
          </div>
        </div>
      </div>
