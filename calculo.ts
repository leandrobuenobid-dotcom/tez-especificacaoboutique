export interface EntradaCalculo {
  ...
  tipoTrafego?: string
  area: number
  perda: number // percentual ex: 10
  usarPrimer?: boolean
  usarFundo?: boolean
  usarTela?: boolean
  usarCera?: boolean
  consumoPrimer?: number // g/m² para piso drenante: 200, 250, 300, 350
...
export function calcular(entrada: EntradaCalculo): ResultadoCalculo {
  ...
  if (entrada.categoriaId === 'cimento') {
    // Aplicação fixa: apenas parede/teto
    // Ambiente: apenas interno (indicação fixa, não requer seleção)
    const rend = 17.5 // m²/balde — igual para Matte e Lunar
    ...
    // Sem primer — apenas cera acrílica como complemento opcional
  ...
  if (entrada.categoriaId === 'drenante') {
    ...
    // Tela de Fibra de Vidro — sempre necessária para todos os substratos
    // usarTela padrão = true
    ...
    // Primer PU — obrigatório para contrapiso, recomendado para outros
    const primerObrigatorio = tb === 'Contrapiso Leve' || tb === 'Contrapiso Pesado'
    if (entrada.usarPrimer !== false) {
      // consumoPrimer em g/m², padrão 200 g/m²
      ...
      alertas.push('⚠️ ATENÇÃO: O Primer PU é INDISPENSÁVEL para aplicação sobre contrapiso. Recomende ao cliente incluí-lo.')
    }
